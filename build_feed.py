...
def price_to_9(harbuz_price):
    """Гарбуз x 1.07 + округлення за шкалою Едуарда (затверджена 28.08):
       < 20 грн  -> вгору до цілого
       20-50    -> вгору до числа, що закінчується на 0, 5 або 9
       >= 50    -> вгору до числа, що закінчується на 9
    Без цієї шкали товар за 9 грн ставав 19 грн (+111%)."""
    v = math.ceil(float(harbuz_price) * MARKUP)
    if v < 20:
        return v
    if v < 50:
        while v % 10 not in (0, 5, 9):
            v += 1
        return v
    return v + ((9 - v % 10) % 10)
